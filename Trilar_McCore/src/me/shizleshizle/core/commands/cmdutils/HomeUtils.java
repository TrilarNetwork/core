...
public class HomeUtils {
    private static final HashMap<String, ArrayList<String>> homes = new HashMap<>();

    public static void setHome(User p, String name, Location loc) {
        setHome(p, name, loc.getX(), loc.getY(), loc.getZ(), loc.getYaw(), loc.getPitch(), Objects.requireNonNull(loc.getWorld()).getName());
    }
    ...
    public static void saveHomeNames() {
        for (String p : homes.keySet()) {
            Main.c.getHomes().set("homes." + p + ".homeNames", homes.get(p));
        }
        Main.c.saveHomes();
    }

    public static void loadHomes() {
        ConfigurationSection players = Main.c.getHomes().getConfigurationSection("homes");
        if (players != null) {
            for (String path : players.getKeys(false)) {
                ArrayList<String> pl = new ArrayList<>(Main.c.getHomes().getStringList("homes." + path + ".homeNames"));
                homes.put(path, pl);
            }
        }
    }
